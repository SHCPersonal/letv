...
hadoop jar /usr/local/hadoop/share/hadoop/tools/lib/hadoop-streaming-2.6.0.jar -D mapred.output.compress=0 -D mapred.reduce.tasks=0 -input $inpath -output $encode_fea_path -mapper ./mapper_encode_fea.py -file ./mapper_encode_fea.py -file ./model_key -file ./key_id.dict.local
...
then
    ...
    cat predict.res | python ./predict_parser.py > local_out.binary
    echo $training_date >> new_result.txt
    echo "vw predict" >> new_result.txt
    cat local_out.binary | ./scoreKDD.py >> new_result.txt
    #rm -rf predict.res local_out


    rm -rf detail_predict_feature.txt
    hadoop fs -cat $merge_path/part-00000 |./ftrl/ftrl_predict_std model local_out
    echo $training_date >> new_result.txt
    echo "no_vw predict" >> new_result.txt
    cat local_out|./scoreKDD.py >> new_result.txt
    rm -rf detail_predict_feature.txt local_out

    hadoop fs -cat $encode_fea_path/part-00000 | python logistic_predict_model_text.py model.text > predict.raw
    cat predict.raw | python ./predict_parser.py > local_out.raw
    echo $training_date >> new_result.txt
    echo "raw predict" >> new_result.txt
    cat local_out.raw | ./scoreKDD.py >> new_result.txt
fi
###########################
...
ftrl="./vw $bit_precision --ftrl --cache_file temp.cache --l1 0.5 --l2 0.1 --ftrl_alpha 0.05 --ftrl_beta 1.0 --passes 2 --noconstant --hash strings $init --loss_function logistic -f model.binary.new --readable_model model.text.new"
