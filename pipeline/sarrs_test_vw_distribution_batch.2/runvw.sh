...
vwcmd="./vw --total 100 --node $mapper --cache_file temp.cache --span_server 10.148.12.11 --noconstant --l2 0.1 --loss_function=logistic --holdout_off"
mapred_job_id=`echo $mapred_job_id | tr -d 'job_'`
gdcmd="$vwcmd -b 24 --unique_id $mapred_job_id --passes 10 --sgd -d /dev/stdin -f tempmodel"
mapred_job_id=`expr $mapred_job_id \* 2` #create new nonce
bfgscmd="$vwcmd --unique_id $mapred_job_id --bfgs --mem 5 --passes 100 -f model.binary --readable_model model.text -i tempmodel"
if [ "$mapper" == '000000' ]; then
    ...
    outfile=$mapreduce_task_output_dir/model.binary
    ...
    found=`hadoop fs -lsr | grep $mapreduce_task_output_dir | grep model.binary`
    if [ "$found" != "" ]; then
      hadoop fs -rm -r $outfile
    fi
    date +"%F %T outfile=$outfile" > /dev/stderr
    hadoop fs -put model.binary $outfile
    ...
    #outfile=$mapreduce_task_output_dir/model_$mapper
    #outfile_readable=$mapreduce_task_output_dir/model_$mapper.text
    #mapperfile=$mapreduce_task_output_dir/mapperout_$mapper
